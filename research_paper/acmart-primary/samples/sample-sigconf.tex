...
%% article.
\begin{abstract}
State access is increasingly the dominant cost in large-state stream processing, where per-record updates and timer-driven workloads turn random memory accesses into a memory-bound bottleneck. Existing backends often suffer from fragmented object layouts and long indirection chains, causing frequent cache misses that inflate tail latency and cap throughput. This paper presents a cache-friendly state storage design that shortens the access path and improves locality without changing stream-processing semantics or snapshot/restore compatibility. The design combines compact hash indexing with control-byte based parallel matching, an inlined key/value layout that aligns probing and key validation within the same cache line, and a lightweight storage hierarchy that organizes state by key-group and namespace to concentrate working sets. We evaluate the approach on Linux using (i) flink-state-benchmark microbenchmarks for isolated backend behavior and (ii) the benchset end-to-end suite for application-level impact. Across workloads with high update rates and window/timer pressure, results show consistently higher throughput, lower latency, reduced cache-miss intensity, and a clearer shift away from memory-bound execution.
\end{abstract}
