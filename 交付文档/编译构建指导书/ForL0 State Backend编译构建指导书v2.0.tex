...
构建/运行证据全部写入 \texttt{benchmark/results/runs/<run\_id>/}；成功后发布的 \texttt{benchmark/results/latest/} 为扁平文件集合。每轮还自动写入 \texttt{hardware\_snapshot.json} 与 \texttt{l0\_calibration.json}，分别保存目标机硬件/软件指纹以及 128 MiB L0--DRAM 延迟、带宽、工作集和 1/2/4 worker 并发标定曲线。标定程序在 campaign 目录临时编译，执行后删除二进制，只保留小型 JSON。服务器端不构建交付 PDF 或 figure，避免离线 Python/字体/LaTeX 差异污染结果；PDF 在本地从已上传的 raw、JSON 和 log 派生。
...
输出明确标记为 \texttt{simulation/model}，用于本地调参和相对趋势预测；交付性能数字仍必须来自真实 L0 服务器的 \texttt{real-online} 作业。
