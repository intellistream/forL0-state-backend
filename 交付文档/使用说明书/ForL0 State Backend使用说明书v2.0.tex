...
\texttt{state.backend.forl0.max-table-capacity} & int & 1024 & 单张 SwissTable 的容量上限，达到后触发分裂。 \\
\bottomrule
\end{tabularx}
\end{table}
...
任一项不满足时，HotCache 快路径默默保持未激活；状态访问由底层 SwissTable 直接服务，作业可正常启动与运行。运行时可以通过 Flink 指标系统的 \texttt{forl0.hotcache.active} gauge 确认是否真正激活（见 \S7.3）。
...
\texttt{state.backend.forl0.max-table-capacity}     & 1024 & 单表容量上限，达到后触发分裂而非继续扩容。 \\
...
启用 L0 Hot-Key Cache 时，后端在作业的 \texttt{MetricGroup} 下自动注册以下 gauge，指标名为 \texttt{<operator>.forl0.hotcache.<field>}，可直接在 Flink Web UI 或对接的指标后端（Prometheus / InfluxDB 等）中观察：
...
当设置 \texttt{l0-cache.enabled=true} 但 \texttt{active} 始终为 0 时，说明硬件门禁未通过（见 \S3.3），作业已自动回退到纯 SwissTable 路径；此时 \texttt{bytesCapacity=0}、\texttt{hits=0}、\texttt{lookups=0}。
