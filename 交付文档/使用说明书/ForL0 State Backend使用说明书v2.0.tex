...
\project{} 是面向 Apache Flink 的高性能 Keyed State 后端，针对鲲鹏服务器的体系结构特性进行了定向优化。系统以 C++ 原生引擎承担状态的权威存储与访问，以 Java 控制层与 JNI 桥接层承担 Flink 接口适配与状态注册，在保持与 Flink 原生状态接口完全兼容的前提下，通过 SwissTable 哈希结构、SWAR 并行匹配以及可选的 L0 Cache 加速，缩短热点状态访问路径的延迟。
...
\begin{itemize}
  \item 完全兼容 Flink Keyed State 接口与语义，现有作业代码无需改动。
  ...
  \item 支持 Flink 原生 Checkpoint 与 Savepoint 机制，覆盖增量快照与恢复。
  \item 原生 Copy-On-Write 快照，异步快照不阻塞前台状态访问。
\end{itemize}
...
运行模式 & L0 模式（需 \texttt{/dev/hisi\_l0} 与 \texttt{libl0mempool.so}）/ 模拟模式（自动回退） \\
\bottomrule
\end{tabularx}
\end{table}
...
\project{} 对 Flink 所有 Keyed State 接口提供原生支持，具体映射关系如下：

\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{L{3.4cm} X L{4.4cm}}
\toprule
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[accentbox, text width=11.6cm] (native) at (8.4,0.7) {NativeEngine + StateTable + SwissTable + L0 Allocator};
...
\centering
\footnotesize
\begin{tabularx}{\textwidth}{L{7.0cm} L{1.8cm} L{1.4cm} X}
\toprule
\textbf{配置项} & \textbf{类型} & \textbf{默认值} & \textbf{说明} \\
\midrule
\texttt{state.backend.type} & String & -- & 填写 \texttt{StateBackendFactory} 的全限定类名以启用本后端，具体值见 \S 3.1。 \\
\texttt{state.backend.forl0.async-snapshots} & boolean & \texttt{true} & 使用原生 Copy-On-Write 异步快照。 \\
\texttt{state.backend.forl0.l0-cache.enabled} & boolean & \texttt{false} & 是否启用 L0 Cache 加速，需鲲鹏硬件支持。 \\
\texttt{state.backend.forl0.l0-cache.size} & MemorySize & 256\,mb & L0 内存池总容量，支持 \texttt{256mb}、\texttt{1gb} 等写法。 \\
\texttt{state.backend.forl0.l0-cache.total-size} & MemorySize & -- & 主机/设备级 L0 总预算；按 \texttt{expected-engines} 划分给 TaskManager 进程。 \\
\texttt{state.backend.forl0.l0-cache.expected-engines} & int & 1 & 预期进程级 manager 数；应填写 TaskManager 进程数，不是 slot 或 StateEngine 数。 \\
\texttt{state.backend.forl0.l0-cache.strict-allocation} & boolean & \texttt{false} & 为 true 时，硬件门禁或分配失败直接使作业失败，适用于正式 L0 证据采集。 \\
\texttt{state.backend.forl0.native-memory.max-size} & MemorySize & 0 & 单 StateEngine 的 SwissTable native 内存上限；0 表示不设限。 \\
\texttt{forL0.metricsCollector.enabled} & boolean & \texttt{false} & 是否注册 HotCache gauge；性能复现默认关闭，诊断时显式开启。 \\
\texttt{state.backend.forl0.initial-table-capacity} & int & 64 & 单张 SwissTable 的初始 slot 数，必须为 2 的幂且 $\ge 8$。 \\
\texttt{state.backend.forl0.max-table-capacity} & int & 1024 & 单张 SwissTable 的容量上限，达到后 fail-fast，防止无界内存增长。 \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{L0 模式启用条件}
...
\midrule
\texttt{state.backend.forl0.initial-table-capacity} & 64 & 初始 slot 数，必须为 2 的幂且 $\ge 8$。 \\
\texttt{state.backend.forl0.max-table-capacity}     & 1024 & 单表容量上限，达到后 fail-fast 而非无界扩容。 \\
\multicolumn{2}{l}{负载因子} & 内部常量 87.5\%，触发 rehash / grow 的负载阈值。 \\
\bottomrule
\end{tabularx}
\end{table}
...
\project{} 与 Flink 状态接口保持严格兼容。用户作业中状态的声明、读写、清理方式与使用 HashMapStateBackend 时完全一致。
...
SQL 作业通过集群配置启用 \project{} 即可，运行方式与使用其他状态后端时没有差别。例如下列窗口聚合自动落在 \project{} 的时间窗口路径上：
...
对于 SQL 场景中常见的 Deduplication、Join、Aggregate 和窗口算子，\project{} 会自动识别 \texttt{VoidNamespace}、\texttt{TimeWindow} 等类型，并选择对应的快路径，无需额外配置。
...
\project{} 使用原生 Copy-On-Write 机制实现异步快照，与 Flink 的 Checkpoint 框架完全衔接。用户侧的配置方式和操作方式保持不变。
...
\begin{itemize}
  ...
  \item 从其他状态后端（如 HashMapStateBackend）迁移到 \project{} 时，建议从新的 Savepoint 开始运行。canonical Savepoint 在大多数情形下可互通，但由于底层存储结构差异，不建议直接基于老作业的增量 Checkpoint 启动。
...
\texttt{[ForL0-HotCache] L0 cache disabled by config.} & 当前 StateEngine 明确采用 L0-off；WordCount p4 融合计数探针属于此类 \\
...
\texttt{[ForL0-HotCache] state\_attach name=\ldots} & 受支持状态已从共享池取得 HotSet，是正式 L0-on 证据之一 \\
\texttt{[ForL0-HotCache] WARN: L0 hardware not available \ldots} & 配置要求 L0，但设备、库或分配门禁失败；strict 正式实验不得接收 \\
...
当设置 \texttt{l0-cache.enabled=true} 但 \texttt{active} 始终为 0 时，说明硬件门禁未通过（见 \S3.3），非 strict 作业已回退到纯 SwissTable 路径；此时 \texttt{bytesCapacity=0}、\texttt{hits=0}、\texttt{lookups=0}。若 \texttt{lookups=0,hits=0} 但日志显示 L0 已激活，则表示该 workload 没有调用 HotCache get，命中率应记为“不适用（0/0）”而不是 0\%。例如 WordCount \texttt{addAndGetLong} 在一次 JNI 内直接修改权威 SwissTable，只做更新而不读取 HotCache，因此正式 p4 探针会主动关闭 L0。
...
\toprule
\textbf{状态规模} & \texttt{l0-cache.size} & \textbf{说明} \\
\midrule
小（\textless\,100\,MB 状态） & 64\,mb & 轻量作业，大部分热键能被 HotSet 覆盖。 \\
中（100\,MB -- 1\,GB）   & 256\,mb（默认） & 典型业务作业，推荐从默认值开始。 \\
大（\textgreater\,1\,GB） & 512\,mb 及以上 & 与单 NUMA 可用 L0 资源匹配，目前单 NUMA 约 20\,MB 上限。 \\
\bottomrule
\end{tabularx}
\end{table}
...
\project{} 的状态数据位于原生内存，不占用 JVM 堆空间，也不依赖 Flink Managed Memory。据此可作如下调整：

\begin{itemize}
  \item 适当增加 \texttt{taskmanager.memory.process.size} 的上限，以容纳原生状态数据。
  \item 相应降低 \texttt{taskmanager.memory.managed.size}，为原生数据面让出空间。
  \item 若开启 L0 Cache，确保 \texttt{l0-cache.size} 与节点实际可用的 L0 资源相匹配。
\end{itemize}
\subsection{Checkpoint 间隔}

\project{} 的异步快照不阻塞前台访问，对吞吐的影响主要来自序列化与写出阶段。建议：
...
在完整仓库覆盖到离线服务器 \texttt{/root/forL0} 后，只需执行：

\begin{lstlisting}[language=shellcmd]
cd /root/forL0
chmod +x reproduce-all stop-reproduce-all
./reproduce-all
\end{lstlisting}

脚本会后台运行并立即返回 PID，先完成四项隔离 smoke，再执行完整正式矩阵。监控和停止命令分别为：

\begin{lstlisting}[language=shellcmd]
tail -f benchmark/results/runs/*/.logs
./stop-reproduce-all
\end{lstlisting}

运行中或失败时，完整证据位于 \texttt{benchmark/results/runs/<run\_id>/}；失败目录会保留 \texttt{FAILED.txt}、\texttt{.logs} 和局部结果用于诊断，但不会发布为最新有效数据。全部成功后，脚本将 campaign 扁平发布到 \texttt{benchmark/results/latest/}，该目录只包含文件而没有子目录，适合直接通过网页上传；\texttt{UPLOAD\_MANIFEST.tsv} 记录每个扁平文件对应的原始相对路径。

正式矩阵包括 WordCount W01--W02、NexMark N01--N14 和 Client C01--C08。NexMark 默认成对覆盖 q3/q4/q9/q18/q19/q20，是代表性性能集合；仓库虽提供 q0--q22 的 SQL 与可选场景，但 \texttt{reproduce-all} 不等价于全 23 query 穷举。服务器只产出 raw、JSON 与日志证据，不生成 figure/PDF；报告和图应在联网本地取得 latest 后生成。

若脚本来自 Windows 或网页中转，出现 \texttt{/usr/bin/env: bash\textbackslash r}，说明文件被转换为 CRLF。应重新以二进制方式覆盖仓库文件；仓库的 \texttt{.gitattributes} 会固定 shell 脚本为 LF。直接执行前还应保留 executable bit，或运行上面的 \texttt{chmod +x}。
...
\centering
\small
\begin{tabularx}{\textwidth}{L{4.8cm} L{3.8cm} X}
\toprule
...
\texttt{ClassNotFoundException: ForL0StateBackendFactory} & JAR 未部署 & 确认 \texttt{\$FLINK\_HOME/lib/} 下存在 \texttt{flink-statebackend-forL0-*.jar} \\
日志中 \texttt{l0Enabled=false} & L0 硬件/库不可用或未启用 & 核查 \texttt{/dev/hisi\_l0}、\texttt{libl0mempool.so} 以及 \texttt{l0-cache.enabled} \\
Checkpoint 失败 & Checkpoint 目录权限不足 & 确认 \texttt{state.checkpoints.dir} 路径对 Flink 进程可写 \\
\texttt{forl0.hotcache.active=0} 但配置已开启 & 硬件门禁未通过 & 检查 \texttt{/dev/hisi\_l0} 是否存在，\texttt{libl0mempool.so} 是否在搜索路径上 \\
正式 L0-on 场景出现 \texttt{disabled by config} & 场景覆盖或配置传递错误 & 检查 workload 的 \texttt{forl0\_overrides}；WordCount p4 出现该日志属于预期，pressure/late-query 场景则不应出现 \\
...
作业 OOM & 状态规模超出进程内存预算 & 增加 \texttt{taskmanager.memory.process.size}，或设置状态 TTL \\
\bottomrule
\end{tabularx}
\end{table}
