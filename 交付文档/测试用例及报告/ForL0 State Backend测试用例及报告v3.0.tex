...
\begin{itemize}
  ...
  \item \textbf{L0 Cache 是 ForL0 的硬件增强层}。从状态访问路径看，NexMark 的端到端收益首先来自权威状态路径从 JVM 堆上对象图迁移到堆外 \texttt{StateTable}/\texttt{SwissTable}；HotCache 主要进一步加速白名单内热点标量 \texttt{ValueState}，对窗口、TopN、Join 中的 RowData、聚合器和内部状态结构不改变语义边界。
...
\textbf{机制层面的解释}：ForL0 不是在 Flink 原生 HeapStateBackend 外侧增加一层缓存，而是将权威状态路径收敛到 \texttt{StateEngine -> StateTable -> KeyGroup -> Namespace -> SwissTable}。Java 层负责 Flink 接口与生命周期控制，底层 \texttt{StateTable}/\texttt{SwissTable} 承担权威 KV 存储。与 HashMapStateBackend 相比，该路径减少 \texttt{CopyOnWriteStateMap}、\texttt{HashMap.Node}、装箱对象、复合 key 对象和 GC promotion 成本，因此 q4/q5/q9/q18/q20 在无 Full GC 放大效应的复跑口径下仍能形成端到端收益。
