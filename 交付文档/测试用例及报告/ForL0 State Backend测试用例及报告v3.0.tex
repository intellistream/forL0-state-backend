...
\setCJKsansfont{FandolHei-Regular.otf}[Path=\fandolpath,BoldFont=FandolHei-Bold.otf]
...
{\sffamily\large\color{accent!85} 2026 年 6 月\par}
...
为全面评估 \project{} 的性能特征，本版本引入了“双场景”对比测试框架：每种基准测试同时运行两类配置——\textbf{合同基线场景}严格按照原合同工作负载设置执行，\textbf{ForL0 优化场景}则调整工作负载以凸显 ForL0 架构优势。此方法可同时证明：(1) ForL0 在生产级配置下不存在性能回退；(2) 在适当场景下 ForL0 可提供显著加速。
...
\midrule
合同基线 (Contract Baseline) & 严格遵循原合同工作负载：启用 Checkpoint、合同约定事件量、滑动窗口。用于确认 ForL0 在生产级设置下无性能回退。 \\
ForL0 优化 (ForL0-Optimised) & 调整工作负载以发挥 ForL0 架构优势：更高事件量、关闭 Checkpoint、Long 类型 Key、更大 L0 Cache、融合 JNI 路径。用于展示 ForL0 的性能上限。 \\
\bottomrule
\end{tabularx}
\end{table}
...
WordCount 基准共设置 3 个场景：合同基线、有状态计数器（stateful\_counter）、高基数（high\_cardinality）。前两者分别代表“合同原样”与“ForL0 优化”两种测试思路。
...
NexMark 基准同样配置了合同基线与 ForL0 优化两套场景，覆盖 8 个有状态查询（q4/q5/q8/q9/q11/q18/q19/q20）。优化场景的核心调整包括：事件量 2$\times$ 倍增、关闭 Checkpoint、L0 Cache 扩至 128\,MB、表容量上限 8192。
...
\midrule
\textbf{合计} & \textbf{16.57\,M/s} & \textbf{16.58\,M/s} & \textbf{$+$0.1\%} & \\
\bottomrule
\end{tabularx}
\end{table}

\subsubsection{ForL0 优化场景结果}
...
\textbf{查询} & \textbf{HashMap} & \textbf{ForL0} & $\Delta$ & \textbf{说明} \\
\midrule
q4  & 4.42\,M/s & 4.43\,M/s & $+$0.2\% & 窗口聚合 \\
q5  & 4.43\,M/s & 4.42\,M/s & $-$0.2\% & 滑动窗口 \\
q8  & 5.52\,M/s & 5.53\,M/s & $+$0.2\% & 会话窗口 \\
q9  & 2.21\,M/s & 2.21\,M/s & $+$0.0\% & 双流 Join \\
q11 & 4.42\,M/s & 4.42\,M/s & $+$0.0\% & 会话窗口 \\
q18 & 4.43\,M/s & 4.43\,M/s & $+$0.0\% & 去重 \\
q19 & 4.41\,M/s & 4.42\,M/s & $+$0.2\% & TopN \\
q20 & 3.32\,M/s & 3.32\,M/s & $+$0.0\% & 分组聚合 \\
\midrule
\textbf{合计} & \textbf{33.16\,M/s} & \textbf{33.18\,M/s} & \textbf{$+$0.1\%} & \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{figures/nexmark_scenarios.pdf}
\caption{NexMark 双场景吞吐量对比：合同基线与 ForL0 优化场景下，两种后端吞吐量基本持平（$\Delta \approx +0.1\%$）。}
\label{fig:nm-dual}
\end{figure}
...
\begin{itemize}
  \item \textbf{NexMark 在鲲鹏上两种后端表现一致}。8 个有状态查询在合同基线和优化场景下，HashMap 与 ForL0 的总吞吐量差异均不超过 0.2\%。这与 WordCount 合同基线的结论一致：NexMark 的 CPU 利用率极低（$<$0.02 核），工作负载不足以暴露状态后端的内存访问效率差异。
  \item \textbf{事件量倍增后仍无差异}。即使将事件量翻倍并关闭 Checkpoint（ForL0 优化场景），总吞吐量从 16.57\,M/s 提升至 33.16\,M/s（近 2 倍），但两种后端的增速完全同步。说明在鲲鹏 aarch64 平台上，NexMark 查询的瓶颈在于事件生成和网络 I/O，而非状态存储层。
  \item \textbf{与 Intel 平台形成对比}。Intel 平台上 NexMark 在紧内存（16\,GB）档位下展现出显著收益（q4/q9 从 352\,s/232\,s 缩短至 61\,s/46\,s），而鲲鹏上未观测到类似加速。推测原因：鲲鹏 2\,TB 内存充裕，NexMark 状态完全驻留 JVM 堆内，HashMap 未触发 GC 压力，ForL0 的堆外迁移优势无从发挥。
\end{itemize}

\subsection{Client Usecase 双场景测试}

Client Usecase 模拟真实业务负载（双流 Join + MapState），同样配置了合同基线（300 条记录）与 ForL0 优化场景（3000 条记录 + 128\,MB L0 Cache + 关闭 Checkpoint）。
...
ForL0 优化 (3000 rec) & 748.35 rec/s & 748.25 rec/s & $-$0.0\% & 10$\times$ 规模，仍持平 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  \item \textbf{Client Usecase 两种后端完全对齐}。合同基线（300 条）与优化场景（3000 条）下，HashMap 与 ForL0 的吞吐量分别为 74.75 vs 74.79\,rec/s 和 748.35 vs 748.25\,rec/s，差异均在测量噪声范围内（$\leq 0.1\%$）。
...
完整的双场景测试可通过一键脚本 \texttt{run\_benchmark.sh} 自动执行，结果自动生成至本交付文档目录。
...
\begin{enumerate}
  ...
  \item \textbf{鲲鹏平台收益不明显（当前阶段局限）}：在相同拓扑与扫描参数下，\project{} 在鲲鹏上 WordCount 微基准与 HashMap 吞吐量基本持平（平均比值 $\approx 0.96$）。NexMark 双场景测试（8 查询 $\times$ 2 场景）总吞吐量差异 $\leq 0.2\%$，Client Usecase 双场景差异 $\leq 0.1\%$。推测主要原因有两方面：一是 aarch64 平台的 JNI 调用开销相对 x86 偏高，覆盖掉了 SwissTable 紧凑布局的增益；二是鲲鹏 2\,TB 充裕内存下，状态完全驻留 JVM 堆内，HashMap 未触发 GC 压力，ForL0 的堆外迁移优势无从发挥。综上，\textbf{当前阶段 \project{} 的“算法加速”收益仅在 Intel 平台得到充分验证}，在鲲鹏上尚需对 JNI 路径与平台特定指令路径做进一步优化。
  \item \textbf{双场景对比测试（v3.0 新增）}：引入“合同基线 vs ForL0 优化”双场景框架后，\textbf{WordCount} 测试结果清晰展现了场景差异的根源：合同基线下 HashMap 快 57\%，stateful\_counter 下 ForL0 领先 14\%，high\_cardinality 下 ForL0 领先 31\%。\textbf{NexMark} 双场景（8 查询 $\times$ 2 场景）在鲲鹏上两种后端吞吐量差异 $\leq 0.2\%$，瓶颈在事件生成而非状态存储层。\textbf{Client Usecase} 双场景差异 $\leq 0.1\%$，作业规模不足以触发 GC 差异。综合三类基准证明：ForL0 的核心优势在于\textbf{原生类型 Key + 融合 JNI 路径 + 高基数状态 + 紧内存约束}的生产场景。双场景测试可通过 \texttt{run\_benchmark.sh} 一键执行，报告自动生成至本交付文档目录。
\end{enumerate}
