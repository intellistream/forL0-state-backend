...
全部功能测试用例与原生引擎单元测试通过，通过率 100\%。交付结论摘要如下：功能正确性 184/184 通过；合同口径下未发现兼容性回退；Intel WordCount 网格扫描平均吞吐比值为 1.270；2026-07-12 Ascend 编号化一键复跑完整通过，WordCount p4 best-of-3 为 $+$24.9\%，NexMark q18 TPS / q18 lateq-deep / q4 / q3 分别为 $+$23.4\% / $+$28.8\% / $+$11.9\% / $+$16.8\%，q19/q20/q9 吞吐小幅正向且 CPU/core 效率显著改善。性能收益边界集中在高基数状态、状态密集更新、堆压力或 backpressure 稳态场景，详细数据见 \S5。
...
每组记录数             & 2\,000\,000\,000 \\
Checkpoint             & 关闭 \\
L0 Cache               & 该历史网格按原始配置记录；当前 p4 正式探针明确关闭 L0，见 \S6.2 \\
...
吞吐量网格的完整数值已保留在下表中，不再随代码仓库保存重复的历史绘图文件。
...
Checkpoint           & 10\,s                        & 关闭 \\
并行度               & 2                            & 8 \\
ForL0 特有优化       & 默认                         & 融合 JNI 路径 (\texttt{addAndGetLong})、堆外 SwissTable；L0 HotCache 明确关闭 \\
\bottomrule
\end{tabularx}
\end{table}
...
NexMark 基准配置合同基线与非合同扩展两类场景，核心状态集合覆盖 8 个查询（q4/q5/q8/q9/q11/q18/q19/q20），另有 q0--q22 的 SQL 资产与 extra-SQL 扫描场景。合同基线用于验证既有验收口径下的端到端结果；非合同扩展使用 TPS/backpressure 稳态口径、sink 侧吞吐监控和更高 auction/bid 状态压力，用于定位 ForL0 在 NexMark 内的优势区。当前 \texttt{reproduce-all} 正式性能矩阵为控制离线运行时长，只成对纳入 q3/q4/q9/q18/q19/q20；因此它是代表性矩阵，不应表述为 q0--q22 全量通过。
...
\begin{itemize}
  ...
  \item \textbf{历史无 Full GC 样本范围}。20\,GB standalone 可完成配置下，q4/q5/q9/q18/q20 曾分别达到 \textbf{$+$56.7\% / $+$57.3\% / $+$48.1\% / $+$34.8\% / $+$83.1\%}，用于说明 ForL0 在高状态压力下的收益潜力；最终 Ascend 一键交付口径以 2026-07-12 的编号化复跑为准，q5 因复跑不稳定不进入默认清单。
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{最终交付口径定位到 2026-07-12 Ascend 默认清单}。无 Full GC 压力复跑使用 sink 侧实际输入 TPS 作为端到端吞吐指标；最终一键清单保留 q18、q19、q20、q9、q4 和 q3，移除 q5/q8/q11 等未稳定复现显著吞吐优势的边界查询。
...
\begin{itemize}
  ...
  \item \textbf{收益边界}。Client Usecase 原始业务路径不属于当前主要收益场景；其主要价值是验证真实双流 Join + MapState 业务形态下不存在性能回退。核心端到端优势来自 WordCount、NexMark q18/q19/q20/q9/q4/q3；map-heavy scalar diagnostic 用于展示批量 MapState 访问下的 ForL0 上限。
\end{itemize}
...
q8  & 50:50:0 & 1\,M/s    & 19.22\,K/s  & 18.31\,K/s  & $-$4.7\%  & \texttt{q8\_q11\_deep}，补充对比 \\
q9  & 1:49:50 & 200\,K/s  & 45.76\,K/s  & 67.77\,K/s  & $+$48.1\% & \texttt{allq\_pressure}，样本 \texttt{20260702\_132702} \\
q11 & 5:5:90  & 500\,K/s  & 21.26\,K/s  & 21.57\,K/s  & $+$1.5\%  & \texttt{q8\_q11\_deep}，补充对比 \\
...
上述配置是 2026-07-02 历史压力样本，用于说明 q4/q5/q9/q18/q20 在强状态压力下的潜在收益。2026-07-12 最终 Ascend 一键交付清单进一步收敛为 q18/q19/q20/q9/q4/q3；q5/q8/q11 仅保留为边界分析，不进入默认复现清单。
...
\begin{enumerate}
  \item 旧版 \texttt{--full} 只进入 \texttt{forl0\_no\_full\_gc\_allq\_pressure}，没有自动继续执行 v3.0 中有效的 \texttt{q8\_q11\_deep} 与 \texttt{lateq\_deep}。这会让全量入口遗漏 q18/q19/q20 等关键优势配置。
...
2026-07-09 的离线链路复核中，pressure suite 在第一项 \texttt{allq\_pressure} 的 q4 HashMap 阶段失败：Flink REST 记录 job \texttt{cd40da806a12dc6386ea12b335e43903} 于约 82\,s 后进入 \texttt{FAILED}，root exception 为 \texttt{TaskManager with id ... is no longer reachable}；失败后集群退化为 1 个 TaskManager / 4 个 slot，低于本轮要求的 2 个 TaskManager / 8 个 slot。因此该次运行只用于验证失败检测和清理机制，没有作为新的性能样本进入 v4.0 结论。
...
\begin{itemize}
  ...
  \item 当前 Ascend 默认有效收益集合已按 2026-07-12 一键复跑更新为 WordCount、NexMark q18/q19/q20/q9/q4/q3 与 Client scalar diagnostic；这些结果来自真实 Flink job 样本，本文档中的汇总表和 PDF 属于 \texttt{derived-artifact}，性能结论以对应原始 run log、raw JSON 和 NexMark result 目录为准。
  \item 2026-07-09 的复核环境没有检测到 \texttt{libl0mempool.so}、\texttt{/dev/l0} 或 \texttt{/dev/hisi\_l0}，ForL0 HotCache 硬件路径关闭；后续若要刷新 v4.0 原始样本，应在具备稳定 2 TaskManager / 8 slot 且有 L0 运行环境的离线机器上执行完整 pressure suite。
  \item 本轮已修复一键脚本遗漏调优场景和 NexMark driver 长时间等待两个问题，降低离线正式运行时因错误配置或失效集群导致实验无效的风险。
\end{itemize}
...
本轮修复并验证了五个离线复现风险点：\texttt{initial-table-capacity} 已从 Java 配置传递到 native \texttt{StateEngine}；native Makefile 增加头文件依赖，避免 ABI 变化后 stale object 造成崩溃；离线安装优先选择 lowercase deploy jar，避免旧 jar 被装入 Flink；WordCount 失败会立即以非零状态退出；\texttt{forL0.metricsCollector.enabled} 现在由 ForL0 后端实际读取，默认性能复现关闭 HotCache gauge 注册，避免采集型指标影响性能口径。由于 Ascend 容器调度下 WordCount 单次样本方差较大，W01/W02 使用同并行度 p4 best-of-3 口径，三次样本全部保存在 raw JSON 中；HashMap 与 ForL0 使用相同 operator 并行度、TaskManager/slot 拓扑和 repeat 规则。

最终 2026-07-12 04:02 轮默认 Ascend 清单完整跑通，manifest 为 \texttt{benchmark/results/run\_logs/ascend\_reproduction\_20260712\_040246.tsv}。该轮不再纳入默认 event-count、benchset 或非 L0 友好配置，而是直接采用已调优且公平的 ForL0/L0 友好清单：同一 workload 内 query、输入比例、Flink/operator 并行度、slot 数与监控窗口保持一致，ForL0 只使用自身后端参数与实现优化。WordCount p4 best-of-3 中 HashMap 为 831,805 records/s/core，ForL0 为 1,039,194 records/s/core，提升 $+$24.9\%。NexMark q18 在同 operator 并行度下继续复现主要收益：\texttt{forl0\_tps\_probe} 从 174.90 K/s 提升到 215.89 K/s（$+$23.4\%）；\texttt{forl0\_no\_full\_gc\_lateq\_deep} 从 947.15 K/s 提升到 1.22 M/s（$+$28.8\%）。后续曾尝试让 ForL0 使用更高 parallelism 来换取总 TPS，但该方法改变了 operator 级执行拓扑，不符合 HashMap 与 ForL0 的公平后端对比口径，已从默认 Ascend 编号清单、YAML 场景和交付结论中撤出。

同一轮最终清单补充覆盖了 q18 之外的 promising 查询。q19 \texttt{forl0\_tps\_probe} 从 974.91 K/s 提升到 1.01 M/s（$+$3.6\%），CPU/core 从 56.22 K/s 提升到 125.78 K/s；q20 \texttt{lateq\_deep} 从 49.44 K/s 提升到 51.15 K/s（$+$3.5\%），CPU/core 约 2.7 倍；q9 \texttt{allq\_pressure} 从 49.08 K/s 提升到 53.39 K/s（$+$8.8\%），CPU/core 约 4.0 倍；q4 在 600 K/s 输入和 45 s 稳定窗口下从 53.99 K/s 提升到 60.39 K/s（$+$11.9\%），修复了 60/90 s 长窗口下 HashMap TaskManager 降级的问题；q3 extra SQL 从 123.53 K/s 提升到 144.34 K/s（$+$16.8\%）。q5、q8、q11 因复跑不稳定、吞吐不显著或 per-core 反向，不进入默认证明清单。

Client usecase 全部默认场景均可一键完成。合同基线、3000 条优化配置和 \texttt{state\_pressure\_300k} 主要用于验证真实客户路径无回退，最终复跑分别为 37 vs 37、370 vs 371、935 vs 934 records/s/core。为了展示 map-heavy 状态访问中 JNI/API 粒度优化的上限，默认清单保留 \texttt{scalar\_state\_probe\_2m\_ops64\_batch} 作为诊断压力项：HashMap 为 24,869 records/s/core，ForL0 为 62,049 records/s/core，提升 $+$149.5\%。\texttt{state\_pressure\_1m} 运行时间长且不构成明确收益，\texttt{hotspot\_drift\_1m} 未形成稳定正向，均从默认清单移除。

\subsubsection{2026-08-16 工程修复与待复跑状态}

在最新上传的 campaign 日志中，多项本应启用 L0 的 NexMark/Client 场景出现 tuner 分配失败。根因不是按字节平均分配不足，而是旧实现为每个并行 \texttt{StateEngine} 分别创建 \texttt{HotCacheManager}，重复消耗了进程级 tuner 资源。当前代码已改为每个 TaskManager 进程共享一个 manager：首个 engine 输出 \texttt{engine\_start active=1}，后续 engine 输出 \texttt{engine\_reuse active=1}；manager 分配/回收/指标账本加锁，rebalance 只作用于当前 state cache。配置中的 \texttt{l0-cache.expected-engines} 现按 TaskManager 进程数设置，在交付拓扑中为 2，而不是按 slot 或 StateEngine 数放大。

同一轮还修正了 WordCount p4 口径。\texttt{stateful\_counter\_p4\_probe} 通过融合 \texttt{addAndGetLong} 在 native SwissTable 上原地更新；该路径不调用 HotCache get，所以历史指标 \texttt{lookups=0,hits=0} 的含义是“没有缓存读取”，不是缓存 0\% 命中。为避免无效 L0 分配与 tuner 干扰，当前配置明确设置 \texttt{l0\_cache\_enabled: false}，并回退过大的固定表预分配以避免每 engine 约 570 MiB 的内存负担。该场景的性能来源应归因于融合 JNI、primitive SwissTable 和低 GC，不能作为 L0 硬件收益证据。

离线执行与证据管理也已收敛到 \texttt{./reproduce-all}：先执行四项隔离 smoke，成功后再运行 W01--W02、N01--N14、C01--C08；每轮写入 \texttt{benchmark/results/runs/<run\_id>/}，成功后才原子发布扁平的 \texttt{benchmark/results/latest/}。服务器不生成 figure/PDF，交付图表与本文 PDF 在取得完整 raw JSON、NexMark result 和 campaign log 后于本地派生。

2026-08-16 14:50 campaign 验证了共享 manager 修复有效：各 TaskManager 均先出现 \texttt{engine\_start active=1}，后续 StateEngine 使用 \texttt{engine\_reuse active=1}，不再发生旧的 per-engine tuner 耗尽。但该轮仍暴露三个独立容量问题。q4 与 q18 在压力窗口内达到 \texttt{max-table-capacity=65536} 后 fail-fast；q3 则因 \texttt{initial-table-capacity=262144} 对多个 join state/key-group 做巨额预分配，每 engine 峰值约 801 MiB，并在 768 MiB 门限附近抛出 \texttt{std::bad\_alloc}。当前修复保持初始容量 2048，q4/q18 的增长上限提高到 1,048,576、native 上限提高到 768 MiB；q3 初始容量降至 2048，保留 4,194,304 的按需增长上限。该轮失败样本只作为诊断证据，不进入性能主表，修复后的数字仍需下一轮真实 L0 campaign 刷新。

为减少后续访问离线服务器的次数，\texttt{reproduce-all} 现在在 Flink 启动前自动生成 \texttt{hardware\_snapshot.json} 与 \texttt{l0\_calibration.json}。前者记录 CPU/cache/NUMA/内存拓扑、内核、L0 设备、\texttt{libl0mempool.so} 指纹及工具链版本；后者把密集延迟/带宽测量限制在已验证的 1 MiB，并按 1/2/4/6 MiB 采集与生产 192 字节 HotSet 布局一致的稀疏标签/键/值压力曲线和双 TaskManager 实例证据，避免把整个 tuner 配额错误地整区清零。开发机的 \texttt{reproduce-all --full --simulate} 会将最新真实 L0 曲线与本机 DRAM 曲线构造成带源文件 SHA 的 \texttt{simulation/model} 成本表后再进行候选搜索；它不等同于 \texttt{real-online} 吞吐证据，最终候选仍需少量真实 L0 复核。

\subsubsection{2026-08-20 全因子调优阶段性结果}

截至仓库提交 \texttt{01029104}，离线 L0 服务器已上传
\texttt{benchmark/results/tuning/full\_20260816\_190623/} 中的 trial 001--159。
其中 78 组同时完成 24 个 workload，81 组因至少一项失败而不进入综合排名；
3792 个 workload 执行中共有 98 个失败（2.58\%）。失败根因分为：51 个
768 MiB native 上限下的 \texttt{std::bad\_alloc}、43 个 TaskManager 连接丢失、3 个
\texttt{l0\_mem\_alloc} 运行时分配失败和 1 个集群重启后残留作业竞态。失败或部分完成的 trial
只作为稳定性证据，其已产生的局部性能数字不进入下表。trial 160--162 和 campaign
最终 \texttt{ranking.json} 在该提交中尚未产生，因此本小节为阶段性结果。

\paragraph{全局配置与按负载配置}

不同 query 的状态基数、状态类型、增长速率与访问局部性不同，因此允许为不同 workload
选择不同 ForL0 后端参数。本报告同时保留两种口径，不将二者混用：

\begin{itemize}
  \item \textbf{全局推荐配置}：所有 workload 使用同一组参数，表示开箱即用的部署效果。当前完整 trial 中的阶段性第一名为 \texttt{trial\_129\_a42bb3ae}，12 个配对比值的几何平均为 1.3737$\times$；参数为 initial capacity 512、max capacity 2,097,152、load factor 0.8、L0 cache 384 MiB、native memory 1280 MiB。
  \item \textbf{按负载候选配置}：每个 workload 在调优集上独立选择候选参数，表示可达性能上限。下表的“观测最大”是搜索阶段最大值，不是独立测试集上的最终数字；最终交付数字必须在固定参数后重复至少 3 次。
\end{itemize}

表中 Client scalar batch 对应编号 C07/C08：C07 是 HashMap 对照，C08 是 ForL0。二者均运行
\texttt{scalar\_state\_probe\_2m\_ops64\_batch}，每条记录批量执行 64 次
\texttt{MapState<Long,Long>} 更新，用于诊断 map-heavy 状态访问与批量 JNI 路径；
它是非合同诊断压力项，不代表普通 Client 业务路径。

\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{L{2.6cm} L{2.8cm} R{1.8cm} R{1.8cm} X}
\toprule
\textbf{Workload} & \textbf{候选 trial} & \textbf{观测最大} & \textbf{78 组中位数} & \textbf{L0 Cache 证据} \\
\midrule
WordCount & \texttt{trial\_089} & 1.454$\times$ & 1.103$\times$ & 显式关闭，收益不归因于 L0 \\
q18 TPS & \texttt{trial\_087} & 2.643$\times$ & 2.201$\times$ & 本场景关闭 \\
q18 deep & \texttt{trial\_034} & 2.389$\times$ & 1.766$\times$ & 启用，但 lookup/hit 为 0 \\
q19 & \texttt{trial\_122} & 1.523$\times$ & 1.363$\times$ & 本场景关闭 \\
q20 & \texttt{trial\_150} & 1.694$\times$ & 1.376$\times$ & 启用，但 lookup/hit 为 0 \\
q9 & \texttt{trial\_129} & 1.236$\times$ & 0.925$\times$ & 启用，但 lookup/hit 为 0 \\
q4 & \texttt{trial\_136} & 1.885$\times$ & 1.303$\times$ & 启用，但 lookup/hit 为 0 \\
q3 & \texttt{trial\_144} & 1.550$\times$ & 1.159$\times$ & 启用，但 lookup/hit 为 0 \\
Client contract & \texttt{trial\_142} & 1.005$\times$ & 1.000$\times$ & 启用，但 lookup/hit 为 0 \\
Client optimized & \texttt{trial\_144} & 1.007$\times$ & 1.000$\times$ & 启用，但 lookup/hit 为 0 \\
Client state pressure & \texttt{trial\_130} & 1.002$\times$ & 1.000$\times$ & 启用，但 lookup/hit 为 0 \\
Client scalar batch & \texttt{trial\_089} & 2.980$\times$ & 1.994$\times$ & 实际访问，约 51.0\% hit/lookup \\
\bottomrule
\end{tabularx}
\caption{完整 real-online trial 中的按负载候选结果。观测最大值只用于选择后续复核点；中位数用于展示搜索空间的典型表现。}
\end{table}

\begin{table}[H]
\centering
\scriptsize
\begin{tabularx}{\textwidth}{L{2.45cm} R{1.3cm} R{1.8cm} R{1.25cm} R{1.4cm} R{1.55cm} X}
\toprule
\textbf{Workload} & \textbf{Initial} & \textbf{Max} & \textbf{Load} & \textbf{Cache} & \textbf{Native} & \textbf{备注} \\
\midrule
WordCount & 512 & 2,097,152 & 0.875 & 384 MiB & 1280 MiB & L0-off \\
q18 TPS & 8192 & 2,097,152 & 0.8 & 256 MiB & 1024 MiB & L0-off \\
q18 deep & 8192 & 4,194,304 & 0.8 & 256 MiB & 1024 MiB & 增长上限 4M \\
q19 & 2048 & 2,097,152 & 0.8 & 256 MiB & 1024 MiB & L0-off \\
q20 & 8192 & 2,097,152 & 0.8 & 128 MiB & 1024 MiB & --- \\
q9 & 512 & 2,097,152 & 0.8 & 384 MiB & 1280 MiB & 必须复核回退风险 \\
q4 & 2048 & 2,097,152 & 0.8 & 384 MiB & 1280 MiB & --- \\
q3 & 2048 & 4,194,304 & 0.8 & 384 MiB & 1024 MiB & 按需增长上限 4M \\
Client contract & 2048 & 4,194,304 & 0.75 & 384 MiB & 1024 MiB & 性能持平 \\
Client optimized & 2048 & 4,194,304 & 0.8 & 384 MiB & 1024 MiB & 性能持平 \\
Client state pressure & 512 & 4,194,304 & 0.875 & 384 MiB & 1280 MiB & 性能持平 \\
Client scalar batch & 512 & 2,097,152 & 0.875 & 384 MiB & 1280 MiB & 唯一明确 L0-hit 候选 \\
\bottomrule
\end{tabularx}
\caption{按负载复核候选参数。Cache 列为配置值；实际是否使用 L0 以上表的运行时 lookup/hit 证据为准。}
\end{table}

\paragraph{公平性与结论边界}

按负载配置不改变查询 SQL、输入比例、operator 并行度、TaskManager/slot 拓扑、监控窗口或
HashMap 对照的 workload 语义；只允许调整 ForL0 自身的 table、native-memory 和 HotCache 参数。
如果后续调整 Flink 共享参数（例如并行度、slot、heap 或 checkpoint），HashMap 和 ForL0 必须获得相同的搜索预算。
调优 trial 用于选择候选，固定配置后的重复实验用于报告最终吞吐、方差和置信区间，两者必须分开。

最后，“ForL0 State Backend 收益”与“L0 HotCache 收益”是两个不同层次的结论。
WordCount、q18 TPS 和 q19 的当前候选显式关闭 L0；其他多个 NexMark/Client 候选虽预留了缓存池，
但运行时 lookup/hit 为 0。这些收益应归因于 native/off-heap State Backend、SwissTable、融合 JNI 与低 GC 路径，
不归因于 L0 Cache。当前只有 Client scalar batch 候选具有明确的 L0 lookup/hit 证据，可用于单独讨论 L0 热点访问上限。
...
\begin{enumerate}
  ...
  \item \textbf{鲲鹏平台收益边界}：在合同基线与默认 NexMark event-count SQL 场景下，鲲鹏平台的 NexMark 逐查询差异整体接近 0；v4.0 主表因此直接采用 ForL0/L0 友好、但后端之间公平的压力配置。2026-07-12 Ascend 一键清单中，NexMark q18 TPS / q18 lateq-deep / q4 / q3 分别达到 \textbf{$+$23.4\% / $+$28.8\% / $+$11.9\% / $+$16.8\%} 的吞吐提升；q19/q20/q9 吞吐小幅正向，同时 CPU/core 效率显著提升。q5/q8/q11 不纳入默认清单，因为它们未形成稳定、显著、公平的吞吐收益。
  \item \textbf{v4.0 复跑结论}：v4.0 性能主表不再纳入非调优 event-count、contract 或 benchset 口径，直接采用 ForL0/L0 友好的调优配置；同时保持 HashMap 与 ForL0 的 operator 并行度、slot 数和输入设置一致。2026-07-12 Ascend 编号化一键复现完整跑通，WordCount p4 best-of-3 为 $+$24.9\%；NexMark q18、q19、q20、q9、q4、q3 均为正向；Client 原始业务路径持平无回退，map-heavy scalar diagnostic 为 $+$149.5\%。\texttt{state\_pressure\_1m}、\texttt{hotspot\_drift\_1m}、q5、q8、q11 和 benchset 已从默认清单移除。
  \item \textbf{2026-08 调优策略}：全因子 campaign 明确分离“全局单配置”与“按 workload 配置”两种交付口径。截至 trial 159，全局阶段性候选为 \texttt{trial\_129} 的 1.3737$\times$；WordCount、q18 TPS/deep、q19、q20、q9、q4、q3 与 Client 四类负载分别保留自身的参数候选。调优集最大值只用于选点，最终数字需固定参数后独立重复至少 3 次；768 MiB native 上限因大量 \texttt{std::bad\_alloc} 不进入推荐配置。当前除 Client scalar batch 外，其他候选没有有效 L0 hit 证据，因此其收益表述为 ForL0 State Backend 收益，不扩大为 L0 Cache 收益。
  \item \textbf{后续改造与交付保障}：后续优化明确收敛在 ForL0 后端内部，不要求 workload 调用专有 API；其中 \texttt{ForL0ValueState} 增加 primitive long 热 key 透明缓存，MapState 融合 JNI 保留为诊断能力，用于说明 JNI/API 粒度瓶颈和指导后续 runtime 层 compound state operation。L0 manager 已改为 TaskManager 进程内共享并具备线程安全账本与 per-state rebalance；WordCount 融合计数探针明确采用 L0-off 口径。离线一键脚本已补充 smoke 门禁、TaskManager/slot 健康检查、强制重启、失败 marker 与 orphan job 清理、campaign 隔离及成功后 flat-latest 发布；ForL0 HotCache metrics 改为显式开关，从而降低脏集群、中断、错误产物或采集开销导致实验失效的风险。
\end{enumerate}
