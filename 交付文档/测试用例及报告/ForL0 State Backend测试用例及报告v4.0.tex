...
\begin{enumerate}
  ...
  \item \textbf{v4.0 复跑结论}：v4.0 性能主表不再纳入非调优 event-count、contract 或 benchset 口径，直接采用 ForL0/L0 友好的调优配置；同时保持 HashMap 与 ForL0 的 operator 并行度、slot 数和输入设置一致。2026-07-11 Ascend 编号化一键复现已完整跑通：WordCount fastpath 为 $+$20.0\%，NexMark q18 总吞吐在 TPS probe / promising 两种配置下分别为 $+$4.8\% / $+$23.4\%，Client \texttt{state\_pressure\_1m} 与 \texttt{hotspot\_drift\_1m} 分别为 $+$1.5\% / $+$4.6\%。q19/q20 在同并行度 Ascend 复测中没有形成稳定显著总吞吐收益，已从默认 headline 清单移除。
