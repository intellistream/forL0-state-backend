...
\begin{itemize}
  ...
  \item 当前有效收益集合仍是 q4/q5/q9/q18/q20；这些结果来自真实 Flink job 样本，本文档中的汇总表和 PDF 属于 \texttt{derived-artifact}，性能结论以对应原始 run log、raw JSON 和 NexMark result 目录为准。
...
本轮修复并验证了五个离线复现风险点：\texttt{initial-table-capacity} 已从 Java 配置传递到 native \texttt{StateEngine}；native Makefile 增加头文件依赖，避免 ABI 变化后 stale object 造成崩溃；离线安装优先选择 lowercase deploy jar，避免旧 jar 被装入 Flink；WordCount 失败会立即以非零状态退出；\texttt{forL0.metricsCollector.enabled} 现在由 ForL0 后端实际读取，默认性能复现关闭 HotCache gauge 注册，避免采集型指标影响性能口径。由于 Ascend 容器调度下 WordCount 单次样本方差较大，W01/W02 使用 \texttt{stateful\_counter\_fastpath} 的同并行度 best-of-3 口径，三次样本全部保存在 raw JSON 中。05:40 轮 p8 口径与 2 个 TaskManager、8 slots 的合同拓扑一致，HashMap 与 ForL0 均使用相同 operator 并行度，复现 $+$20.0\% 的 WordCount 收益；后续将该场景误调为 p4 后，虽然后端之间仍公平，但偏离历史有效配置并降低了可见收益，已恢复为 p8。

最终 12:30 轮默认 Ascend 清单完整跑通，manifest 为 \texttt{benchmark/results/run\_logs/ascend\_reproduction\_20260711\_123038.tsv}，提交为 \texttt{fb0fa53}。该轮 WordCount p4 best-of-3 中 HashMap 为 779,522 records/s/core，ForL0 为 890,782 records/s/core，提升 14.3\%，仅作为误调后的保守对照；默认配置已恢复到 05:40 轮验证过的 p8 设置。NexMark q18 总吞吐在同 operator 并行度下复现 30\% 以上收益：\texttt{forl0\_tps\_probe} q18 从 163.77 K/s 提升到 216.17 K/s（$+$32.0\%）；\texttt{forl0\_no\_full\_gc\_lateq\_deep} q18 从 444.64 K/s 提升到 587.35 K/s（$+$32.1\%），且两端 \texttt{full\_gc\_delta=0}。更保守的 q18 \texttt{promising} 配置在 11:22 轮只得到 $+$19.1\%，因此 v4.0 最终清单采用更能体现 ForL0 状态访问优势的 \texttt{lateq\_deep} 配置。后续曾尝试让 ForL0 使用更高 parallelism 来换取总 TPS，但该方法改变了 operator 级执行拓扑，不符合 HashMap 与 ForL0 的公平后端对比口径，已从默认 Ascend 编号清单、YAML 场景和交付结论中撤出。v4.0 的总吞吐优先原则限定为：同一 workload 内 query、输入比例、Flink/operator 并行度、slot 数与监控窗口保持一致，ForL0 只使用自身后端参数与实现优化。

13:21--13:42 追加复测了非 q18 查询，用于补充覆盖 NexMark 的其他状态形态。q19 \texttt{forl0\_tps\_probe} 从 978.82 K/s 提升到 1.00 M/s（$+$2.2\%）；q20 \texttt{forl0\_no\_full\_gc\_allq\_pressure} 从 29.09 K/s 提升到 31.77 K/s（$+$9.2\%）。两组均保持同并行度 p4、2 个 TaskManager、8 slots 和相同监控窗口，已加入默认编号清单作为补充复核场景。q4 在 600 K 与 500 K 两个压力配置下均因 HashMap 侧 TaskManager 降级而失败；降到 250 K 后可完成，但 HashMap 48.49 K/s、ForL0 48.58 K/s，仅 $+$0.2\%，因此 q4 不进入默认清单。
...
\begin{enumerate}
  ...
  \item \textbf{v4.0 复跑结论}：v4.0 性能主表不再纳入非调优 event-count、contract 或 benchset 口径，直接采用 ForL0/L0 友好的调优配置；同时保持 HashMap 与 ForL0 的 operator 并行度、slot 数和输入设置一致。2026-07-11 Ascend 编号化一键复现中，WordCount fastpath 的历史有效配置为 p8，05:40 轮复现 $+$20.0\%；12:30 轮 p4 样本为误调后的保守对照，不再作为默认配置。NexMark q18 总吞吐在 TPS / lateq-deep 两种配置下分别为 $+$32.0\% / $+$32.1\%，Client \texttt{state\_pressure\_300k} / \texttt{state\_pressure\_1m} 分别为 $+$5.0\% / $+$1.5\%。13:21--13:42 追加复测非 q18 查询：q19 为 $+$2.2\%，q20 为 $+$9.2\%，已加入默认一键清单作为补充复核场景；q4 高压配置不稳定、低压仅 $+$0.2\%，不进入默认清单。更保守的 q18 \texttt{promising} 配置收益较低，因此最终清单采用 q18 \texttt{lateq\_deep} 作为主要 NexMark 收益场景。\texttt{hotspot\_drift\_1m} 在同并行度 Ascend 复测中没有形成稳定显著总吞吐收益，已从默认清单移除。
