...
为减少后续访问离线服务器的次数，\texttt{reproduce-all} 现在在 Flink 启动前自动生成 \texttt{hardware\_snapshot.json} 与 \texttt{l0\_calibration.json}。前者记录 CPU/cache/NUMA/内存拓扑、内核、L0 设备、\texttt{libl0mempool.so} 指纹及工具链版本；后者在与单 TaskManager 预算一致的 128 MiB 工作集上测量 L0 与 DRAM 的随机读延迟、顺序读写带宽、工作集曲线和 1/2/4 worker 并发扩展。开发机可用 \texttt{calibrate-local-l0-model} 生成本机曲线，再用 \texttt{compare\_l0\_calibrations.py} 构造 \texttt{simulation/model} 成本表。该模型用于本地筛选容量、并行度和热点策略；它不等同于 \texttt{real-online} 吞吐证据，最终候选仍需少量真实 L0 复核。
