...
全部功能测试用例与原生引擎单元测试通过，通过率 100\%。交付结论摘要如下：功能正确性测试 201/201 通过；合同口径下未发现兼容性回退；Intel WordCount 网格扫描平均吞吐比值为 1.270；鲲鹏最终性能矩阵中，WordCount p4 best-of-3 为 $+$24.9\%，NexMark q18 TPS / q18 lateq-deep / q4 / q3 分别为 $+$23.4\% / $+$28.8\% / $+$11.9\% / $+$16.8\%，q19/q20/q9 吞吐小幅正向且 CPU/core 效率显著改善。性能收益边界集中在高基数状态、状态密集更新、堆压力或 backpressure 稳态场景，详细数据见 \S5。
...
每组记录数             & 2\,000\,000\,000 \\
Checkpoint             & 关闭 \\
L0 Cache               & 本组网格不用于 L0 硬件收益结论；p4 正式结果明确关闭 L0 \\
...
\textit{注：标记 $\star$ 的项为 HashMapStateBackend 下因 Full GC 过度导致的大幅拖慢。运行环境：TaskManager Heap 11.7\,GB，Managed 512\,MB，TM 限制 4 核。}
...
因此，q19/q20 的配置选择需要区分两个目标：合同完成性验证用于给出完整端到端结果；TPS/backpressure 稳态实验用于观察状态后端在持续压力下的资源效率差异。ForL0 的优势并非来自更轻量的查询定义，而是来自状态驻留与访问路径的改变；在状态压力升高时，同一查询可以避开 HashMapStateBackend 的堆上对象膨胀和 GC 退化区间。
...
NexMark 基准配置合同基线与非合同扩展两类场景，核心状态集合覆盖 q4/q5/q8/q9/q11/q18/q19/q20，另补充其他有状态 SQL。合同基线用于验证既有验收口径下的端到端结果；非合同扩展采用 TPS/backpressure 稳态口径和更高状态压力，用于评估状态后端在持续负载下的吞吐与资源效率。交付性能矩阵成对运行 q3/q4/q9/q18/q19/q20，是代表性集合，不等同于 q0--q22 全量覆盖。
...
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\begin{tabularx}{\textwidth}{C{0.75cm} C{1.35cm} R{1.2cm} R{1.65cm} R{1.65cm} R{1.2cm} X}
\toprule
\textbf{Q} & \textbf{事件比例} & \textbf{输入 TPS} & \textbf{HashMap} & \textbf{ForL0} & $\Delta$ & \textbf{CPU 核数（HashMap / ForL0）} \\
\midrule
q4  & 1:49:50  & 600\,K/s & 65.64\,K/s  & 102.88\,K/s & \textbf{$+$56.7\%} & 89.51 / 42.77 \\
q5  & 1:1:98   & 5\,M/s   & 9.84\,K/s   & 15.48\,K/s  & \textbf{$+$57.3\%} & 5.68 / 5.38 \\
q8  & 50:50:0  & 1\,M/s   & 19.22\,K/s  & 18.31\,K/s  & $-$4.7\%  & 26.00 / 16.92 \\
q9  & 1:49:50  & 200\,K/s & 45.76\,K/s  & 67.77\,K/s  & \textbf{$+$48.1\%} & 61.76 / 23.01 \\
q11 & 5:5:90   & 500\,K/s & 21.26\,K/s  & 21.57\,K/s  & $+$1.5\%  & 5.45 / 8.45 \\
q18 & 1:9:90   & 12\,M/s  & 446.12\,K/s & 601.20\,K/s & \textbf{$+$34.8\%} & 50.56 / 6.56 \\
q19 & 1:1:98   & 2.4\,M/s & 998.10\,K/s & 1.02\,M/s   & $+$2.2\%  & 18.96 / 7.46 \\
q20 & 1:49:50  & 700\,K/s & 28.78\,K/s  & 52.69\,K/s  & \textbf{$+$83.1\%} & 77.92 / 13.27 \\
\bottomrule
\end{tabularx}
...
\begin{tabularx}{\textwidth}{C{0.65cm} C{1.2cm} R{0.9cm} R{1.4cm} R{1.4cm} R{1.1cm} R{1.45cm} X}
\toprule
...
\midrule
WordCount p4 best-of-3 & 831,805 rec/s/core & 1,039,194 rec/s/core & \textbf{$+$24.9\%} \\
...
q19 每核吞吐从 56.22 K/s 提升到 125.78 K/s；q20 每核效率约为 HashMap 的 2.7 倍；q9 每核效率约为 HashMap 的 4.0 倍。Client 普通业务路径保持持平；map-heavy scalar 诊断场景从 24,869 提升到 62,049 records/s/core，变化为 $+$149.5\%。

WordCount p4 明确关闭 L0 HotCache，其收益来自融合 JNI、primitive SwissTable 和低 GC。多个 NexMark 场景虽配置缓存容量，但运行时 \texttt{lookup/hit} 为 0；这些结果归因于 ForL0 State Backend 整体实现，不归因于 L0 HotCache。Client scalar batch 的 \texttt{hit/lookup} 约为 51.0\%，具有明确的 L0 访问证据。

\subsection{阶段性调优结果}

全因子调优阶段共评估 159 组参数，其中 78 组完成全部 24 个 workload。累计 3792 次 workload 执行中有 98 次失败，失败率为 2.58\%；失败包括 51 次 native 内存分配失败、43 次 TaskManager 连接丢失、3 次 L0 分配失败和 1 次集群残留作业冲突。部分完成的参数组不进入综合排名。
...
\centering
\small
\begin{tabularx}{\textwidth}{L{3.6cm} R{2.2cm} R{2.2cm} X}
\toprule
\textbf{Workload} & \textbf{阶段观测最大} & \textbf{完整组中位数} & \textbf{L0 Cache 证据} \\
\midrule
WordCount & 1.454$\times$ & 1.103$\times$ & 显式关闭 \\
q18 TPS & 2.643$\times$ & 2.201$\times$ & 显式关闭 \\
q18 deep & 2.389$\times$ & 1.766$\times$ & 启用但无 lookup/hit \\
q19 & 1.523$\times$ & 1.363$\times$ & 显式关闭 \\
q20 & 1.694$\times$ & 1.376$\times$ & 启用但无 lookup/hit \\
q9 & 1.236$\times$ & 0.925$\times$ & 启用但无 lookup/hit \\
q4 & 1.885$\times$ & 1.303$\times$ & 启用但无 lookup/hit \\
q3 & 1.550$\times$ & 1.159$\times$ & 启用但无 lookup/hit \\
Client 普通路径 & 1.007$\times$ & 1.000$\times$ & 无有效命中 \\
Client scalar batch & 2.980$\times$ & 1.994$\times$ & 约 51.0\% hit/lookup \\
\bottomrule
\end{tabularx}
\caption{全因子调优阶段结果}
\end{table}

阶段观测最大值用于选择候选参数，不作为最终交付性能数字。候选参数固定后仍需独立重复测试。除 Client scalar batch 外，其余阶段候选没有有效 L0 hit 证据，相应收益归因于 ForL0 State Backend 整体实现。
...
\begin{enumerate}
  ...
  \item \textbf{鲲鹏平台结果}：最终性能矩阵保持 HashMap 与 ForL0 的 operator 并行度、slot 数和输入设置一致。WordCount p4 best-of-3 为 $+$24.9\%；NexMark q18 TPS、q18 lateq-deep、q4、q3 分别为 $+$23.4\%、$+$28.8\%、$+$11.9\%、$+$16.8\%；q19/q20/q9 吞吐正向且每核效率提高。q5/q8/q11 未形成稳定、显著的吞吐优势，不进入最终矩阵。
  \item \textbf{L0 收益边界}：WordCount p4 与部分 NexMark 场景没有有效 HotCache lookup/hit，其收益归因于 native/off-heap、SwissTable、融合 JNI 与低 GC。Client scalar batch 的 hit/lookup 约为 51.0\%，可用于说明 L0 热点访问效果。
