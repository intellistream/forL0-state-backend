...
05:40 轮默认清单全部完成。WordCount best-of-3 中 HashMap 为 519,607 records/s/core，ForL0 为 623,592 records/s/core，提升 20.0\%。NexMark 总吞吐的稳定正收益集中在 q18：\texttt{forl0\_tps\_probe} q18 从 187.45 K/s 提升到 196.48 K/s（$+$4.8\%），\texttt{forl0\_no\_full\_gc\_promising} q18 从 473.19 K/s 提升到 582.39 K/s（$+$23.1\%）。q19/q20 在本轮总 TPS 为持平或小幅负波动，但 ForL0 使用显著更少的 CPU core 维持接近吞吐：q19 TPS probe 的 throughput/core 从 70.23 K/s/core 提升到 116.49 K/s/core，q19 promising 从 66.67 K/s/core 提升到 130.15 K/s/core，q20 promising 从 998/s/core 提升到 6.97 K/s/core。因此报告将 q19/q20 归类为资源效率样本，而非总吞吐 headline。

Client usecase 全部场景均可一键完成。合同基线和 3000 条优化配置基本持平；\texttt{state\_pressure\_1m} 从 930.45 records/s/core 提升到 944.62 records/s/core（$+$1.5\%）；默认 C09/C10 已从不稳定的 \texttt{scalar\_state\_probe\_2m\_ops16\_batch} 替换为 \texttt{hotspot\_drift\_1m}，本轮从 903.44 records/s/core 提升到 944.78 records/s/core（$+$4.6\%）。\texttt{scalar\_state\_probe\_2m\_ops16\_batch} 在关闭 metrics 后复测仍仅为 123,395 vs 123,404 records/s/core，保留在 YAML 作为探索项，不再放入默认 Ascend 编号清单。
...
\begin{enumerate}
  ...
  \item \textbf{v4.0 复跑结论}：v4.0 性能主表不再纳入非调优 event-count、contract 或 benchset 口径，直接采用 ForL0/L0 友好的 NexMark no-Full-GC pressure 调优配置。2026-07-11 Ascend 编号化一键复现已完整跑通：WordCount fastpath 为 $+$20.0\%，NexMark q18 总吞吐在 TPS probe / promising 两种配置下分别为 $+$4.8\% / $+$23.1\%，Client \texttt{state\_pressure\_1m} 与 \texttt{hotspot\_drift\_1m} 分别为 $+$1.5\% / $+$4.6\%。q19/q20 在该轮 Ascend 总吞吐为持平或小幅负波动，但 throughput/core 提升显著，保留为资源效率边界样本。
