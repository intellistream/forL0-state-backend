...
Ascend 默认编号清单采用总吞吐优先原则，但该原则不改变公平后端对比边界：同一 workload 内 HashMap 与 ForL0 必须使用相同 query、输入比例、Flink/operator 并行度、slot 数和监控窗口；ForL0 只允许通过自身后端参数和实现优化取得收益。WordCount fastpath、NexMark q18 TPS probe、NexMark q18 \texttt{lateq\_deep} pressure、NexMark q19/q20 非 q18 边界样本、Client \texttt{state\_pressure\_300k}/\texttt{state\_pressure\_1m} 用作明确正收益样本，其中 q18 TPS probe 与 q18 \texttt{lateq\_deep} 已在 2026-07-11 12:30 Ascend 一键复跑中分别达到 $+$32.0\% 与 $+$32.1\% 总吞吐提升；同日追加的 q19 \texttt{tps\_probe} 与 q20 \texttt{allq\_pressure} 分别为 $+$2.2\% 与 $+$9.2\%，用于避免默认清单只覆盖 q18 并界定其他 query 的收益边界。此前默认清单中过早采用更保守的 q18 \texttt{promising} 配置，导致整体 headline 幅度降到 20\% 以下；v3.0 最终实验设计将默认 headline 切回 L0/ForL0 友好的 q18 \texttt{lateq\_deep}，同时保留 q19/q20 非 headline 样本。该调整属于实验编排与后端参数选择，不修改 workload 源码。曾探索过的 ForL0 higher-parallelism q19/q20 配置虽然面向总 TPS，但改变了 operator 级执行拓扑，已从默认编号清单、YAML 场景和交付结论中撤出；q4 高压配置在当前 Ascend 环境中触发 HashMap TaskManager 降级，低压仅近似持平，因此不进入默认编号清单。\texttt{hotspot\_drift\_1m} 在最新同并行度复跑中未稳定正向，也退出默认编号清单；探索中未稳定产生收益的 \texttt{scalar\_state\_probe\_2m\_ops16\_batch} 保留在 YAML 中供后续 JNI/API 粒度分析使用。
