...
\begin{itemize}
  ...
  \item 在 JNI 热路径的最外层，系统可按配置启用基于鲲鹏 L0 Cache 用户态分区的\textbf{L0 Hot-Key Cache}；缓存 miss 以及不符合类型条件的访问直接回落到 \texttt{SwissTable}，不改变数据一致性。
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{HotCacheManager} 与 \texttt{HotCacheLL}：在 L0 Cache 分区上构造热点键值对的 set-associative 读写穿透缓存；该能力默认关闭，显式启用后若硬件门禁未通过则保持关闭，不做 heap 回补。
\end{itemize}
...
当前缓存只附着到 VoidNamespace 下的标量 \texttt{ValueState}：key 为 \texttt{INT64} 或 \texttt{INT32}，value 为 \texttt{INT64}、\texttt{INT32} 或 \texttt{FLOAT64}。复合 key、TimeWindow namespace、变长 value 以及其他状态类型均直接访问权威 SwissTable。
...
\end{itemize}
所有 key 先归一化为 64 位整数并经 64 位混合函数得到散列值 $h$，再沿用 SwissTable 的 $H_1/H_2$ 位分配方式从 $h$ 中切分：低 7 位 $H_2 = h \,\&\, \mathtt{0x7F}$ 用作 tag，高位 $H_1 = h \gg 7$ 用作 set 定位。set 索引由 $set = H_1 \,\&\, (N-1)$ 给出（$N$ 为 2 的幂）。8 个 way 在 aarch64 上使用 NEON \texttt{vceq\_u8} 并行比较，在 x86\_64 上使用 SSE2，其余平台使用标量实现。

\paragraph{查找算法}
给定 \texttt{(kg, ns, key)}，首先将 key 归一化为 64 位整数，再计算混合散列 $h$ 以及 $H_1$、$H_2$，定位到唯一的 set：
\begin{enumerate}
  \item 读取 tags 行中的 8 个 tag，使用 \texttt{vceq\_u8}($tags$, $H_2$) 得到 8 字节匹配掩码，每匹配一个 way 对应一个非零字节。
...
\paragraph{失效算法}
删除状态条目时，按查找算法定位 set 并匹配 way，将命中 way 的 tag 置为 \texttt{EMPTY}，keys / vals 不做清零。状态缓存释放时，其 HotSet 区间归还 \texttt{HotCacheManager} 共享池，由该管理器统一承担 L0 内存的分配与回收。
...
\subsubsection{状态访问优化边界}

状态访问优化封装在 ForL0 后端、JNI 层和 native 状态引擎内部。用户作业继续使用 Flink 标准的 \texttt{value()}、\texttt{update()}、\texttt{get()}、\texttt{put()} 和 \texttt{clear()} 等接口，不依赖专有状态 API。

\begin{itemize}
  \item \textbf{标准接口路径}：状态实现根据预计算的 \texttt{KeyNsStrategy}、\texttt{ListStrategy} 和 \texttt{MapStrategy}，选择 typed JNI、RowData 只读视图或通用序列化路径。
  \item \textbf{内部融合接口}：\texttt{addAndGetLong}、\texttt{mapAddAndGetLongLong} 和顺序批量更新接口用于已知的复合读改写操作，不属于普通用户作业的公开接口约定。
\end{itemize}

类型布局稳定的 primitive key/value、固定宽度 RowData 和 TimeWindow namespace 使用专用路径；generic bytes、复杂对象和迭代器访问保持 serializer 或通用 JNI 路径。L0 HotCache 独立于 Java 状态对象，仅覆盖 VoidNamespace 标量 ValueState 白名单。
...
若状态已附着 L0 Hot-Key Cache，查询先尝试缓存快路径，命中时直接返回；未附着或缓存 miss 时，再根据 key-group 和 namespace 定位对应的 \texttt{SwissTable}。在表内查找时，系统先扫描 ctrl 区中的 \(H_2\) 指纹以筛选候选槽位，再对少量候选槽位执行键比较；若当前探测组内已经出现 EMPTY 槽位，则说明本次查询未命中，可以直接返回空结果。对已附着缓存的状态，SwissTable 命中后同步回填 cache。
...
在 primitive long 计数类场景中，\texttt{ForL0ValueState} 使用 typed JNI 直接访问 native \texttt{StateTable}，避免通用 serializer 路径。启用 L0 HotCache 且状态符合白名单时，缓存位于 native 层并保持读写穿透；checkpoint、savepoint、rescale 和恢复始终以 native \texttt{StateTable} 为权威来源。

对于可以在后端内部安全表达为“加 delta 并返回新值”的场景，\texttt{ForL0ValueState} 提供 \texttt{addAndGetLong} 内部融合接口，对应 native 层的 \texttt{valueAddAndGet*} 系列 JNI 入口。普通作业仍通过 Flink \texttt{ValueState} 接口访问。
...
\texttt{ForL0MapState} 与 \texttt{NativeEngine} 提供 \texttt{mapAddAndGetLongLong} 和 \texttt{mapAddSequentialAndSumLongLong} 内部融合接口，可将已知的单项或批量 long inner-key 读改写合并到 native 层。标准状态访问仍使用 Flink \texttt{MapState} 接口及 typed \texttt{get}/\texttt{put}/\texttt{contains}/\texttt{remove} 路径。
...
\project{} 支持普通 checkpoint 恢复，并支持 canonical savepoint 中 Keyed State 数据的恢复。恢复流程由 \texttt{ForL0KeyedStateBackendBuilder} 统一驱动，主要包括以下步骤：

\begin{enumerate}
  \item 读取状态句柄并恢复 \texttt{KeyedBackendSerializationProxy}。
  \item 执行 key serializer 兼容性校验，重建 state meta info registry。
  \item 普通 checkpoint 按 Flink 格式恢复 priority queue 状态。
  \item 按 key-group 读取 KV 数据块，并写回对应 typed \texttt{StateTable}。
  \item 对 canonical savepoint，将 Keyed State 的规范格式条目翻译为 native 状态表中的键值结构。
\end{enumerate}
...
\project{} 与 Flink 状态接口和容错机制保持兼容，用户侧无需修改即可使用。标准接口路径自动选择 typed JNI、固定宽度 RowData、TimeWindow 或通用序列化实现；L0 HotCache 仅对满足白名单的状态附着，其他状态始终由 native StateTable 和 SwissTable 提供权威存储。
