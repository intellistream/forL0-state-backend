...
由于 Ascend 容器环境下短时 WordCount 单次运行存在明显方差，v3.0 设计将“实验复现口径”也视为交付设计的一部分：编号化一键脚本允许场景声明 \texttt{repeat\_runs} 与 \texttt{repeat\_policy}，当前 WordCount fastpath 场景采用 best-of-3，并在 raw JSON 中保留每次 job id、吞吐和耗时。这一机制不改变后端实现，也不只对 ForL0 生效；HashMap 与 ForL0 使用同一 repeat 规则，从而降低离线机器单次坏样本导致结论翻转的风险。

Ascend 默认编号清单采用总吞吐优先原则，但该原则不改变公平后端对比边界：同一 workload 内 HashMap 与 ForL0 必须使用相同 query、输入比例、Flink/operator 并行度、slot 数和监控窗口；ForL0 只允许通过自身后端参数和实现优化取得收益。WordCount fastpath、NexMark q18 pressure/promising、Client \texttt{state\_pressure\_1m}/\texttt{hotspot\_drift\_1m} 用作明确正收益样本。曾探索过的 ForL0 higher-parallelism q19/q20 配置虽然面向总 TPS，但改变了 operator 级执行拓扑，已从默认编号清单、YAML 场景和交付结论中撤出；q19/q20 在同并行度 Ascend 复跑中未形成稳定显著总吞吐收益，仅保留为历史边界分析。探索中未稳定产生收益的 \texttt{scalar\_state\_probe\_2m\_ops16\_batch} 不再作为默认 C09/C10，而保留在 YAML 中供后续 JNI/API 粒度分析使用。
