...
Ascend 默认编号清单采用分层原则：WordCount fastpath、NexMark q18 pressure、Client \texttt{state\_pressure\_1m}/\texttt{hotspot\_drift\_1m} 用作总吞吐或 records/s/core 正收益样本；NexMark q19/q20 保留为 throughput/core 资源效率样本；探索中未稳定产生收益的 \texttt{scalar\_state\_probe\_2m\_ops16\_batch} 不再作为默认 C09/C10，而保留在 YAML 中供后续 JNI/API 粒度分析使用。
