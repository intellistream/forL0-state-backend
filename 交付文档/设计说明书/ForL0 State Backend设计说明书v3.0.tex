...
离线服务器复现交付报告中的 Ascend 编号化性能清单时，使用仓库根目录下的 \texttt{forl0-offline-app.sh}：
\begin{verbatim}
export FLINK_HOME=/home/shuhao/flink-1.20.3
cd /home/shuhao/forL0-state-backend
./forl0-offline-app.sh --flink-home "$FLINK_HOME" \
  --skip-docker-load --reproduce-ascend --no-report
\end{verbatim}
该命令覆盖 WordCount、NexMark 和 Client usecase 的默认编号场景，结果写入 \texttt{benchmark/results/run\_logs/ascend\_reproduction\_*.tsv}、\texttt{benchmark/results/raw/} 与 \texttt{benchmark/results/nexmark\_*/nexmark\_results.json}。
...
Ascend 默认编号清单采用总吞吐优先原则，但该原则不改变公平后端对比边界：同一 workload 内 HashMap 与 ForL0 必须使用相同 query、输入比例、Flink/operator 并行度、slot 数和监控窗口；ForL0 只允许通过自身后端参数和实现优化取得收益。交付结果分为主要收益场景和补充复核场景：WordCount fastpath、NexMark q18 TPS 与 NexMark q18 \texttt{lateq\_deep} pressure 用于展示稳定吞吐收益，其中 q18 TPS 与 q18 \texttt{lateq\_deep} 已在 2026-07-11 12:30 Ascend 一键复跑中分别达到 $+$32.0\% 与 $+$32.1\% 总吞吐提升；NexMark q19/q20、Client \texttt{state\_pressure\_300k}/\texttt{state\_pressure\_1m} 用于补充覆盖不同查询和客户用例，同日 q19 \texttt{tps\_probe} 与 q20 \texttt{allq\_pressure} 分别为 $+$2.2\% 与 $+$9.2\%。更保守的 q18 \texttt{promising} 配置只得到 $+$19.1\%，因此最终编号清单采用更能体现 ForL0 状态访问优势的 q18 \texttt{lateq\_deep} 配置。该调整属于实验编排与后端参数选择，不修改 workload 源码。曾探索过的 ForL0 higher-parallelism q19/q20 配置虽然面向总 TPS，但改变了 operator 级执行拓扑，已从默认编号清单、YAML 场景和交付结论中撤出；q4 高压配置在当前 Ascend 环境中触发 HashMap TaskManager 降级，低压仅近似持平，因此不进入默认编号清单。\texttt{hotspot\_drift\_1m} 在最新同并行度复跑中未稳定正向，也退出默认编号清单；探索中未稳定产生收益的 \texttt{scalar\_state\_probe\_2m\_ops16\_batch} 保留在 YAML 中供后续 JNI/API 粒度分析使用。
