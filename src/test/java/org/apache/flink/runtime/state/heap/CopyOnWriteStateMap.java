/*
 * Note that this class is here only because it is used in tests for comparison with
 * ForL0neStateMap. It is not used in production code.
 */
...
public class CopyOnWriteStateMap<K, N, S> extends StateMap<K, N, S> {
     ...
     * new sub-map.
     */
    private static final int MIN_TRANSFERRED_PER_INCREMENTAL_REHASH = 4; // rehash时每次至少迁移4条数据
    ...
    private static final StateMapEntry<?, ?, ?>[] EMPTY_TABLE =
            new StateMapEntry[MINIMUM_CAPACITY >>> 1]; // 提前创建好的空表，用于构造函数，第一次resize会变成minimum size

    /** Empty entry that we use to bootstrap our {@link StateEntryIterator}. */
    ...
    private final TreeSet<Integer> snapshotVersions; // 所有正在进行snapshot的版本号
     ...
     */
    private StateMapEntry<K, N, S>[] primaryTable; // 存储数据的主表
     ...
     */
    private StateMapEntry<K, N, S>[] incrementalRehashTable; // 非扩容期间为空表
    ...
    private int stateMapVersion; // 每次snapshot时版本号加一

    /** The highest version of this map that is still required by any unreleased snapshot. */
    private int highestRequiredSnapshotVersion; // 正在进行snapshot中的最大版本号
    ...
    public S get(K key, N namespace) {
        ...
        for (StateMapEntry<K, N, S> e = tab[index]; e != null; e = e.next) {
            ...
            if ((e.hash == hash && key.equals(eKey) && namespace.equals(eNamespace))) {
                ...
                // copy-on-write check for state 即 value 是否为旧版本数据
                if (e.stateVersion < requiredVersion) {
                    ...
                    // entry是新版，直接修改state即可
                    e.stateVersion = stateMapVersion;
                    e.state = getStateSerializer().copy(e.state); // 对state深拷贝，为了不把老版本的state返回给应用层
                }
    ...
    public <T> void transform(
        ...
        // 如果存在，则返回entry，否则创建一个value为null的entry并返回
    ...
    private StateMapEntry<K, N, S> putEntry(K key, N namespace) {
        ...
        // 计算当前对应的Hash值
        final int hash = computeHashForOperationAndDoIncrementalRehash(key, namespace);
        // 选择 primaryTable 或 incrementalRehashTable
        final StateMapEntry<K, N, S>[] tab = selectActiveTable(hash);
        int index = hash & (tab.length - 1);
        ...
        for (StateMapEntry<K, N, S> e = tab[index]; e != null; e = e.next) {
            ...
            if (e.hash == hash && key.equals(e.key) && namespace.equals(e.namespace)) {
                ...
                if (e.entryVersion < highestRequiredSnapshotVersion) {
                    // 说明当前entry版本小于某正在snapshot的版本号，即当前entry中是旧数据，被其他snapshot持有
                    // 于是要为e和e之前的元素创建副本
                    // handleChainedEntryCopyOnWrite 方法创建副本并返回e的新副本
    ...
    private StateMapEntry<K, N, S>[] selectActiveTable(int hashCode) {
        // 计算hashCode对应的桶，如果大于等于rehashIndex说明还没被迁移，访问primary table
        // 注意是 "<rehashIndex" 的桶已经被迁移
    ...
    private StateMapEntry<K, N, S> handleChainedEntryCopyOnWrite(
        ...
        StateMapEntry<K, N, S> current = tab[mapIdx]; // 桶的头节点
        StateMapEntry<K, N, S> copy;

        if (current.entryVersion < required) {
            // 说明被某snapshot引用，需要创建副本，版本为stateMapVersion，说明没有被snapshot引用
