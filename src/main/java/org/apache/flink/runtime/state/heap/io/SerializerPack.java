...
public class SerializerPack<K, N, S> {
    ...
    public void writeKey(K key) throws java.io.IOException {
        keyOut.clear();
        keySer.serialize(key, keyOut);
    }

    public void writeNamespace(N ns) throws java.io.IOException {
        nsOut.clear();
        nsSer.serialize(ns, nsOut);
    }

    public void writeState(S state) throws java.io.IOException {
        valOut.clear();
        if (state != null) {
